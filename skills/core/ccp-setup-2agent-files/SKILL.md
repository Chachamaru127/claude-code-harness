...
`ccp-generate-workflow-files` が生成する AGENTS.md, CLAUDE.md, Plans.md 以外の
...
sed -e "s/{{VERSION}}/${PLUGIN_VERSION}/g" \
    -e "s/{{DATE}}/${TODAY}/g" \
    ~/.claude/plugins/marketplaces/cursor-cc-marketplace/templates/.cursor-cc-version.template \
    > .cursor-cc-version
```
