...
### Step 0: TDD モード判定（最初に実行）

**デフォルト: TDD Strict モード有効**

```
設定確認: tdd.mode
├── "strict" (default) → TDD 強制
├── "recommended"      → TDD 推奨（現状維持）
└── "off"              → TDD 言及なし
```

**TDD Strict モード時の実行フロー:**
...
**TDD Strict の詳細ルール:** [../tdd-strict/SKILL.md](../tdd-strict/SKILL.md)
...
**TDD Strict モード（デフォルト）では、以下は自動適用:**

| 条件 | 対応 |
|------|------|
| [feature] | テストを先に書く（強制） |
| [bugfix] | 再現テストを先に書く（強制） |
| [refactor] | 既存テスト確認 → リファクタ |
| [config], [docs] | TDD スキップ |

**`tdd.mode: "recommended"` の場合のみ提案形式:**
...
### Step 3: 過去の実装パターン検索（Memory-Enhanced）
