...
VibeCoder（技術知識がなくても開発したい人）向けの Cursor ↔ Claude Code 2エージェント協調プラグイン
...
| `/handoff-to-cursor` | PM（Cursor）への完了レポートを作成 | **チーム引き継ぎ** - エージェント間のきれいなコミュニケーション |
| `/setup-2agent` | Cursor と Claude Code 両方をチームワーク用に設定 | **チームセットアップ** - 2エージェント協調を一発で有効化 |
...
Cursor (PM) と Claude Code (Worker) を一緒に使うチーム向け：
...
| エージェント | 役割 | 責任 |
|-------------|------|------|
| **Cursor** | PM | 機能計画、作業レビュー、本番デプロイ |
| **Claude Code** | Worker | コード作成、テスト、ステージングデプロイ |
