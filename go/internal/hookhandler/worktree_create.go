...
func HandleWorktreeCreate(in io.Reader, out io.Writer) error {
	...
	if err != nil || len(data) == 0 {
		// No payload: nothing we can create. Emit nothing and let the
		// runtime fall back to its default git behavior.
		return nil
	}
	...
	repoCWD := strings.TrimSpace(input.CWD)
	if repoCWD == "" || looksLikeHookDecisionJSON(repoCWD) {
		// Malformed cwd (including the legacy decision-JSON-as-cwd bug).
		// We cannot safely derive a path; emit nothing.
		return nil
	}
...
func initWorktreeState(worktreePath, sessionID string) {
	stateDir := filepath.Join(worktreePath, ".claude", "state")
