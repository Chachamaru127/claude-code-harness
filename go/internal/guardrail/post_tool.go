...
var securityPatterns = []securityPattern{
	{
		...
		Message: "Possible embedding of sensitive information directly into a string from environment variables",
	},
	{
	...
	},
	{
		...
		Message: "Detected code setting user input via innerHTML (XSS risk)",
	},
	{
		Pattern: regexp.MustCompile(`(?i)(?:password|passwd|secret|api_key|apikey)\s*=\s*["'][^"']{8,}["']`),
		Message: "Detected hardcoded sensitive information (password/API key)",
	},
}
		...
		if isTest || isConfig {
			...
			if content != "" {
				...
				if len(warnings) > 0 {
					...
					msg := fmt.Sprintf("[v4] Test tampering detection warning\n\nSuspicious pattern detected in %s `%s`:\n\n%s\n\n[Please verify]\nConfirm that this change does not intentionally disable tests or reduce implementation quality.\nIf tampering is confirmed, revert the change.",
