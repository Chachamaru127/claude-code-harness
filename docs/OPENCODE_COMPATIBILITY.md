...
|------|----------------------|----------|--------|
| ディレクトリ | `.claude/commands/` | `.opencode/commands/` | 変換必要 |
| フォーマット | Markdown + YAML | Markdown + YAML | 同じ |
| 変数 | `$ARGUMENTS` | `$ARGUMENTS`, `$1`, `$2` | 互換 |
| frontmatter | `description`, `description-en` | `description`, `agent`, `model`, `subtask` | 調整必要 |
...
│   └── optional/
├── scripts/
│   ├── build-opencode.js        # 変換スクリプト（新規）
│   └── validate-opencode.js     # バリデーション（新規）
├── opencode/                    # opencode用出力（新規）
│   ├── commands/                # 変換後コマンド
│   │   ├── core/
│   │   └── optional/
...
- [Harness MCP Setup](../commands/optional/mcp-setup.md)
