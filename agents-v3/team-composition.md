...
| トークンコスト | 追加なし | 微増 |
| レイテンシ | 追加なし | 微増 |
| Teammate 互換 | 現行 shipped default | 親セッションの permission mode が互換な場合に検証対象 |

#### 現在の扱い

`/breezing` と `/harness-work --breezing` の shipped default は、現時点では `bypassPermissions` のまま維持する。
`--auto-mode` フラグは、親セッションの permission mode が互換な場合に限って rollout を試すための opt-in として扱う:

```bash
/breezing all                 # 現行既定 (bypassPermissions) で全タスク完走
/breezing --auto-mode all     # 互換な親セッションで Auto Mode rollout を試す
/execute --breezing all
```

**制約**: 親セッションまたはサブエージェント frontmatter が `bypassPermissions` のままだと、その permission mode が優先される。
したがって、Auto Mode を本当に既定化するには teammate 実行経路だけでなく、親セッション側の permission 設計も見直す必要がある。hooks と disallowedTools はそのまま維持する。
...
| `--auto-mode` フラグ | opt-in rollout | 親セッション設計を見直した後に安全に有効化するため |

この分離により、配布テンプレートでは未文書化の設定値を避けつつ、現行の shipped behavior を正しく説明できる。Auto Mode は次段の親セッション設計変更で再度検討する。
